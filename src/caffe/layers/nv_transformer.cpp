...
void NVTransformationLayer<Dtype>::visualize_bboxlist(
  ...
  // label the un-augmented ground-truth:
  Mat3b img_vis(vis_pane, img_rect);
  static const Scalar validClassColor(0, 255, 0);
  static const Scalar ignoredClassColor(127, 127, 127);
  ...
  if (this->phase_ == TRAIN) {
    ...
    setLabel(vis_pane, str_info, Point(0, vis_pane.rows - 25));

    // TODO: buffer overrun
    char imagename[100];
    sprintf(imagename, "%s/augment_%lu.png", g_param_.img_header().c_str(), cnt);
    imwrite(imagename, vis_pane);
  }
  else {
